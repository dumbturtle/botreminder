...
def main():
    ...
    reminder_create = ConversationHandler(
        ...
        states={
            "reminder_add_date": [
                RegexHandler('^(Ввести дату)$', calendar_add_date, pass_user_data=True),
                RegexHandler('^([0-9][0-9]-[0-9][0-9]-[0-9][0-9][0-9][0-9])$', reminder_add_date, pass_user_data=True)
                ],
            "calendar_add_day": [
                RegexHandler('^([1-9]|0[0-9]|1[0-9]|2[0-9]|3[0-1])$', calendar_add_day, pass_user_data=True)
                ],
            "calendar_add_month": [
                RegexHandler('^([1-9]|0[0-9]|1[0-2])$', calendar_add_month, pass_user_data=True)
                ],
            "calendar_add_year": [
                RegexHandler('^([0-9][0-9][0-9][0-9])$', calendar_add_year, pass_user_data=True)
                ],
            "calendar_add_hours": [
                RegexHandler('^([0-9]|[0-2][0-9])$', calendar_add_hours, pass_user_data=True)
                ],
            "calendar_add_minutes": [
                RegexHandler('^([0-9]|[0-5][0-9])$', calendar_add_minutes, pass_user_data=True)
                ],
    ...
    reminder_delete = ConversationHandler(
        entry_points=[
            RegexHandler('^(Удалить напоминание)$', reminds_list, pass_user_data=True)
            ],
        states={
            "confirm_remind_for_delete": [
                RegexHandler('^([0-9]|[0-9][0-9])$', confirm_remind_for_delete, pass_user_data=True)
                ],
            "commit_remind_for_delete": [
                RegexHandler('^(Да)$', commit_remind_for_delete, pass_user_data=True)
                ]
        },
        fallbacks=[
            RegexHandler('^(Нет)$', cancel_remind_for_delete, pass_user_data=True),
            MessageHandler(Filters.text | Filters.video | Filters.photo | Filters.document, dontknow, pass_user_data=True)
            ]
    )
